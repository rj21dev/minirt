...
/*   Updated: 2022/07/04 19:24:39 by coverand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color2	calc_specular(t_light *light, t_inter inter)
{
	...
	specular = color2_add(color2_coeff(inter.closest->color, METALNESS), \
						highlight);
...
static t_color2	calc_shadows(t_light *light, t_inter inter, t_scene scene)
{
	...
	lray = create_ray(vec_add(inter.hit_p, \
					vec_mult(inter.hit_n, 0.1)), l_direction);
	if (!intersect_all(scene.elements, lray, &closest, &dist) || \
						dist > vec_len(vec_sub(light->point, inter.hit_p)))
		coeff = fmax(0, dot_prod(inter.hit_n, l_direction));
	return (ft_find_color(light, inter, coeff));
}
...
t_object *closest, double d_min)
{
	...
	shadows = calc_shadows(scene->light, \
	new_inter(hit_p, hit_n, closest, ray), *scene);
